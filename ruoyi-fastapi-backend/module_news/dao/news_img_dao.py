...
from module_news.entity.do.news_img_do import NewsImg
from module_news.entity.vo.news_img_vo import News_imgModel, News_imgPageQueryModel
            ...
            (
                await db.execute(
                    select(NewsImg).where(
                        NewsImg.img_url == news_img.img_url if news_img.img_url else True,
                        NewsImg.news_id == news_img.news_id if news_img.news_id else True,
                    )
                )
            )
            .scalars()
    ...
    async def get_news_img_list(cls, db: AsyncSession, query_object: News_imgPageQueryModel, is_page: bool = False):
        ...
        query = (
            ...
            .where(
                ...
                NewsImg.img_title == query_object.img_title if query_object.img_title else True,
                NewsImg.img_dis == query_object.img_dis if query_object.img_dis else True,
                NewsImg.img_url == query_object.img_url if query_object.img_url else True,
